...
fn debug_to_string<T: Debug>(error: T) -> String {
    use std::fmt::Write;
    let mut msg = String::new();
    if msg.write_fmt(format_args!("{:?}", error)).is_err() {
        msg.clear();
        msg.push_str("write_fmt error");
    }
    msg
}
...
impl From<Serde_Error> for ApiError {
    fn from(e: Serde_Error) -> Self {
        ApiError::json_err(&debug_to_string(e))
    }
}

impl From<Reqwest_Error> for ApiError {
    fn from(e: Reqwest_Error) -> Self {
        ApiError::net_err(&debug_to_string(e))
    }
}

// ----------- defalut camera config -----------------
// 会被camera.json覆盖
pub struct CreateSourceReqConfigWrapper {
    pub cfg: CreateSourceReqConfig
}

lazy_static! {
pub static ref DEFAULT_CREATE_SOURCE_REQ_CONFIG: Arc<RwLock<CreateSourceReqConfigWrapper>> = {
            let cfg = CreateSourceReqConfig{
        enable_face: true,
        enable_vehicle: true,
        jpg_encode_threshold: 300,
        is_loop: true,
        player: 0,
        upload_url: "".to_string(),
        decode_result_queue_size: 100,
        produce: CreateSourceReqConfigProduce {
        bg_width: 0,
        draw_rect_on_bg: true,
        max_targets_in_track: 3,
        size_filter_rate: 0.25,
        remove_vehicle_without_plate: false,
        feature_count: 1,
        min_plate_width: 20,
        plate_binary: true,
        plate_filter_rate: 0.3,
        min_plate_bit_score: 0.2,
        enable_plate: true,
        enable_vehicle_classify: true,
        enable_plate_merge: true,
        enable_portrait: true,
        vehicle_history_max_duration: 60,
        vehicle_history_max_size: 20,
        plate_precision: 2,
        plate_width: 80,
        plate_height: 26,
        max_plate_height: 0.3,
        stationary_iou_threshold: 0.0,
        face_duplicate_threshold: 0.9,
        face_cluster_threshold: 0.6,
        face_merge_threshold: 0.7,
        face_merge_window: 10000,
        min_face_history_quality: 0.5
        },
        face: CreateSourceReqConfigFace {
        left: -1,
        top: -1,
        width: -1,
        height: -1,
        ...
        sample_interval: 4,
        onet_interval: 3,
        angle_tb_threshold: 25,
        angle_lf_threshold: 45,
        display_margin: 0.5,
        display_width: 300,
        enable_display: true,
        expire_duration: 10000,
        silent_ttl: 100,
        max_tracker: 12,
        min_obj_count: 3,
        min_width: 14,
        ...
        ie_margin_left: 10,
        ie_margin_top: 10,
        remove_abnormal_detection: false,
        horizontal_trip_line: 0,
        thresh:Some(vec![0.6, 0.7, 0.7]),
        },
        vehicle: CreateSourceReqConfigVehicle {
        left: -1,
        top: -1,
        width: -1,
        height: -1,
        ...
        horizontal_trip_line: 0,
        store_ratio:0.9,
        activate_station_ratio:0.7
        },
        live_height: 270,
        live_server: 0,
        live_width: 480
        };
        Arc::new(RwLock::new(CreateSourceReqConfigWrapper{cfg}))
    };
}
...
//---------------------------------------- do_post ------------------------
async fn do_post<R: Serialize, T: DeserializeOwned>(client: &Client, url: &str, method: &str, req: &R) -> ApiResult<T> {
    ...
    match rpc_res {
        Output::Success(suc) => {
            Ok(serde_json::from_value::<T>(suc.result)?)
        }
        Output::Failure(fail) => {
            Err(ApiError::jsonrpc_err(fail.error.code.code(), &fail.error.message))
        }
    }
}
...
impl AnalysisApi {
    pub fn new(url: &str) -> Self {
        ...
        headers.insert(header::USER_AGENT, header::HeaderValue::from_static("bm-api/1.0"));
    ...
    pub async fn create_source(&self, src_id: Option<String>, src_url: String, notify_url: String, config: CreateSourceReqConfig) -> ApiResult<CreateSourceRes> {
    ...
    pub async fn update_source(&self, src_id: String, src_url: String, notify_url: String, config: CreateSourceReqConfig) -> ApiResult<UpdateSourceRes> {
...
impl RecognitionApi {
    ...
    pub async fn get_features(&self, images: Vec<String>, retattr: bool) -> ApiResult<GetFeaturesRes> {
        let req = GetFeaturesReq {
            images,
            retattr,
        };
    ...
    pub async fn create_persons(&self, db: String, ids: Vec<String>, features: Vec<Vec<ApiFeatureQuality>>) -> ApiResult<CreatePersonsRes> {
        let req = CreatePersonsReq {
            features,
            ids,
            db,
        };
    ...
    pub async fn get_db_persons(&self, id: String, offset: i64, count: i64) -> ApiResult<GetDbPersonsRes> {
        let req = GetDbPersonsReq {
            id,
            offset,
            count,
        };
    ...
    pub async fn move_persons(&self, db_src: String, db_dst: String, ids: Vec<String>) -> ApiResult<MovePersonsRes> {
    ...
    pub async fn add_features_to_person(&self, db: String, id: String, features: Vec<ApiFeatureQuality>) -> ApiResult<AddFeaturesToPersonRes> {
        let req = AddFeaturesToPersonReq {
            db,
            id,
            features,
        };
        do_post(&self.client, self.url.as_str(), "add_features_to_person", &req).await
    }

    pub async fn add_aggregate_feature_to_person(&self, db: String, id: String, feature: String) -> ApiResult<AddAggregateFeatureToPersonRes> {
        let req = AddAggregateFeatureToPersonReq {
            db,
            id,
            feature,
        };
        do_post(&self.client, self.url.as_str(), "add_aggregate_feature_to_person", &req).await
    }

    pub async fn delete_person_feature(&self, db: String, id: String, face_id: i64) -> ApiResult<DeletePersonFeatureRes> {
        let req = DeletePersonFeatureReq {
            db,
            person: id,
            id: face_id,
        };
        do_post(&self.client, self.url.as_str(), "delete_person_feature", &req).await
    }

    pub async fn search(&self, db: Vec<String>, top: Vec<i64>, threshold: Vec<i64>, features: Vec<Vec<ApiFeatureQuality>>) -> ApiResult<SearchRes> {
        let req = SearchReq {
            features,
            db,
            top,
    ...
    pub async fn compare_n(&self, a: Vec<ApiFeatureQuality>, b: Vec<Vec<ApiFeatureQuality>>) -> ApiResult<CompareNRes> {
        let req = CompareNReq {
            a,
            b,
        };
...
impl FaceNotifyParams {
    ...
    pub fn clear_blob(&mut self, clean_fea: bool) {
        if !self.background.image_buf.is_empty() {
            self.background.image_buf = Bytes::new();
        }

        let _: Vec<()> = self.faces.iter_mut().map(|x| {
            if !x.aligned_buf.is_empty() {
                x.aligned_buf = Bytes::new();
            }

            if !x.display_buf.is_empty() {
                x.display_buf = Bytes::new();
            }
...
type CarPropsType = (i32, Option<String>, Option<String>,
...
impl CarNotifyParams {
    ...
    pub fn get_props_tuple(&self) -> CarPropsType {
        ...
        (move_direct, car_direct, car_color, car_brand, car_top_series, car_series, car_top_type, car_mid_type)
    }
    ...
    pub fn clear_blob(&mut self) {
        ...
        if let Some(x) = self.plate_info.as_mut() {
            if !x.img_buf.is_empty() {
                x.img_buf = Bytes::new();
            }

            if !x.binary_buf.is_empty() {
                x.binary_buf = Bytes::new();
            }
        }
        let _: Vec<()> = self.vehicles.iter_mut().map(|x| {
            if !x.img_buf.is_empty() {
                x.img_buf = Bytes::new();
            }
        }).collect();
