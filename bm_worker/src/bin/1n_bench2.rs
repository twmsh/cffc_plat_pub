...
/// 图片数据为对齐过的
///
async fn do_search_align(api: &bm_api::RecognitionApi, cfg: &Cfg, index: usize) -> std::result::Result<i64, String> {
    let start_time = Local::now();
...
async fn main() {
    ...
    // 开始计时
    let start_time = Local::now();
    let recg_api = Arc::new(bm_api::RecognitionApi::new(config.url.as_str()));

    for i in 0..threads {
        let cfg = config.clone();
        let thread_id = i;
        let api = recg_api.clone();
        ...
        let h = tokio::spawn(async move {
            ...
            if succ != 0 {
                let response_time = total_use as f64 / succ as f64;
                info!("[{}] succ: {}/{}, average time:{}", thread_id, succ, count, response_time);
            } else {
    ...
    info!("");
    info!("{} theads, tasks: {}/{}, duration: {} mills, tps: {}", threads, total_succ, total_task, dur, tps);
    info!("average succ response: {} mills", average_use);
